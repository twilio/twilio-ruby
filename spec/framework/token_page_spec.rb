  ...
  describe '#previous_page' do
    context 'when previous_page_url is available' do
      before do
        ...
        @prev_response = instance_double('Twilio::Response',
                                         body: {
                                           'meta' => {
                                             'key' => 'records',
      ...
      it 'properly handles status code when navigating to previous page' do
        expect(@prev_response.status_code).to eq(200)
        expect(@prev_response.headers).to include('status_code' => '200')
      end
    end
