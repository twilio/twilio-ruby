    ...
    class Page
      ...
      def process_response(response)
        if response.status_code != 200
          raise Twilio::TwilioError.new('Unable to fetch page', response)
        end
      ...
      def load_page(payload)
        ...
        raise Twilio::TwilioError.new('Page Records can not be deserialized')
      end
      ...
      def get_instance(payload)
        raise Twilio::TwilioError.new('Page.get_instance() must be implemented in the derived class')
      end
